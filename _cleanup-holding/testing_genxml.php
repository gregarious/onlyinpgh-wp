...
$query = sprintf("SELECT DISTINCT e.event_name, e.event_id, SUBSTRING_INDEX(e.event_notes, ' ', 30) AS event_notes, e.event_start_date, e.event_end_date, e.event_start_time, e.event_end_time, e.event_slug, CONCAT_WS(',', l.location_address, l.location_town, l.location_state, l.location_postcode, l.location_country) AS event_address, l.location_latitude, l.location_longitude, o.name AS organization_name, o.pic_url AS organization_pic_url, o.link_url AS organization_link_url, o.fan_count AS organization_fan_count, c.name AS community_name, o.type FROM wp_em_events e JOIN wp_em_locations l USING (location_id) JOIN wp_em_organizations o JOIN wp_em_community c WHERE (o.id = e.event_creator) AND (c.community_id = l.location_region) HAVING e.event_start_date >= '$startdate' AND e.event_start_date <= '$enddate' ORDER BY e.event_start_date DESC", 
