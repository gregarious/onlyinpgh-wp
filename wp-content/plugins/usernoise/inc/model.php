...
class UN_Model{
	...
	public function validate_feedback_form($params){
		$errors = array();
		if (get_option(UN_FEEDBACK_FORM_SHOW_SUMMARY) && empty($params['title'])){
			$errors []= __('Please enter a summary.', 'usernoise');
		}
		if (empty($params['description']))
			$errors []= __('Please enter the feedback.', 'usernoise');
		if (get_option(UN_FEEDBACK_FORM_SHOW_EMAIL) && empty($params['email']) && !is_user_logged_in() || 
	...
	public function send_admin_message($id, $params){
		...
		$message = __('A new') . " " . $type . " " . __("has been submitted" . ".") . "\r\n" .
			__('View it:') . " <a href='" . admin_url('post.php?action=edit&post=' . $id) . "'>" .
				admin_url('post.php?action=edit&post=' . $id) . '</a>';
		$message = apply_filters('un_feedback_received_message', $message, $id, $params);
		if (get_option(UN_ADMIN_NOTIFY_ON_FEEDBACK)){
			wp_mail(get_option('admin_email'), __("New") ." " . __($type) . " " .__("submitted at") . " " . get_bloginfo('name'),
