...
if(isset($_GET['sessidpass'])) session_id($_GET['sessidpass']); // workaround for Flash session bug, Quick Post Widget mod
session_start(); // Quick Post Widget mod
$tinybrowser['sessioncheck'] = 'quick-post-widget'; //name of session variable to check, Quick Post Widget mod

// Random string used to secure Flash upload if session control not enabled - be sure to change!
$tinybrowser['obfuscate'] = 's0merand0mjunk!!!222'; // Quick Post Widget mod

// Set default language (ISO 639-1 code)
$tinybrowser['language'] = $_SESSION['qpw_locale']; // Quick Post Widget mod
...
$tinybrowser['path']['image'] = $_SESSION['upath'].'/images/'; // Image files location - also creates a '_thumbs' subdirectory within this path to hold the image thumbnails, Quick Post Widget mod
$tinybrowser['path']['media'] = $_SESSION['upath'].'/media/'; // Media files location, Quick Post Widget mod
$tinybrowser['path']['file']  = $_SESSION['upath'].'/files/'; // Other files location, Quick Post Widget mod
...
$tinybrowser['tinymcecss'] = '../../../../../wp-includes/js/tinymce/themes/advanced/skins/o2k7/dialog.css'; // Quick Post Widget mod
